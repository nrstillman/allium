...
def main(args):
    ...
    if "flow" in args.posterioropt:
        flow_density_estimator_build_fun = posterior_nn(
            model="maf", hidden_features=60, num_transforms=3
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-batch", "--batch_size", type=int, default=50, help="(int)\nBatchsize"
    )
    # io data
    parser.add_argument(
        "-ofo",
    ...
    parser.add_argument(
        "-ofi", "--outputfile", default="c0_", help="(str)\nFolder for data"
    )
    parser.add_argument(
        "-pfo",
        "--posteriorfolder",
